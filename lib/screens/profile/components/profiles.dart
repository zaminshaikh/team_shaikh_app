...
class _ProfilesPageState extends State<ProfilesPage> {
                  ...
                  builder: (context, connectedUsersSnapshot) {
                    ...
                    return StreamBuilder<List<Map<String, dynamic>>>(
                        ...
                        builder: (context, notificationsSnapshot) {
                          ...
                          return buildProfilesPageWithConnectedUsers(
                              context, userSnapshot, connectedUsersSnapshot);
                        });
                  });
            });
  ...
  void extractAndPrintUserInfo(AsyncSnapshot<UserWithAssets> userSnapshot,
    ...
    DateTime dob = user.info['dob'] != null
        ? (user.info['dob'] as Timestamp).toDate()
        : DateTime.now();
    ...
    if (user.info['firstDepositDate'] != null) {
      DateTime firstDepositDateTime =
          (user.info['firstDepositDate'] as Timestamp).toDate();
      userFirstDepositDate =
          DateFormat('MM/dd/yyyy').format(firstDepositDateTime);
    } else {
      userFirstDepositDate = 'N/A';
    }

    initEmail =
        user.info['initEmail'] != null ? user.info['initEmail'] as String : '';
    phoneNumber = user.info['phoneNumber'] != null
        ? user.info['phoneNumber'] as String
        : '';
    address = user.info['address'] as String? ?? '';
    beneficiaryFirstName = user.info['beneficiaryFirstName'] != null
        ? user.info['beneficiaryFirstName'] as String
        : '';
    beneficiaryLastName = user.info['beneficiaryLastName'] != null
        ? user.info['beneficiaryLastName'] as String
        : '';
    ...
    if (connectedUsers.data != null) {
      for (var user in connectedUsers.data!) {
        DateTime? dob = user.info['dob'] != null
            ...
            : null;
        userDobs
            .add(dob != null ? DateFormat('MM/dd/yyyy').format(dob) : 'N/A');
        userEmails.add(user.info['email'] as String? ?? 'N/A'); // Adjusted
        DateTime? firstDepositDate = user.info['firstDepositDate'] != null
            ? (user.info['firstDepositDate'] as Timestamp).toDate()
            : null;
        userFirstDepositDates.add(firstDepositDate != null
            ? DateFormat('MM/dd/yyyy').format(firstDepositDate)
            : 'N/A');
        initEmails.add(user.info['initEmail'] as String? ?? 'N/A'); // Adjusted
        phoneNumbers
            .add(user.info['phoneNumber'] as String? ?? 'N/A'); // Adjusted
        addresses.add(user.info['address'] as String? ?? 'N/A'); // Adjusted
        String beneficiaryFirstName =
            user.info['beneficiaryFirstName'] as String? ?? 'N/A'; // Adjusted
        String beneficiaryLastName =
            user.info['beneficiaryLastName'] as String? ?? ''; // Adjusted
        beneficiaries.add('$beneficiaryFirstName $beneficiaryLastName');
        firstNames
            .add(user.info['name']['first'] as String? ?? 'N/A'); // Adjusted
        lastNames
            .add(user.info['name']['last'] as String? ?? 'N/A'); // Adjusted
        companyNames
            .add(user.info['name']['company'] as String? ?? 'N/A'); // Adjusted
        userNames.add(
            '${user.info['name']['first'] ?? 'N/A'} ${user.info['name']['last'] ?? 'N/A'}'); // Adjusted
        double userTotalAssets =
            0.0; // Ensure this is a double for calculations
        ...
        String formattedTotalAssets =
            NumberFormat('#,##0.00', 'en_US').format(userTotalAssets);
